    ...
    C: Sender + Introspection,
{
        ...
        S: StoreCapabilities,
    {
        ...
        'outer: while let Some(session_present) = self.wait_for_connection().await {
            ...
            if !session_present {
                ...
                // Clear the messages from the previous session
                loop {
                    match self.client_rx.try_recv() {
                        Ok(_) => {
                            trace!("dropped message from client receiver")
                        }
