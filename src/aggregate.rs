...
impl AstarteObject {
    ...
    ///
    /// The value are unique and ordered by insertion. It overrides and returns the previous value
    /// if it's already set.
    pub fn insert(&mut self, name: String, value: AstarteType) -> Option<AstarteType> {
        match self.inner.iter_mut().find(|(item, _)| *item == name) {
    ...
    pub fn remove(&mut self, name: &str) -> Option<AstarteType> {
        let (position, _) = self.inner.iter().find_position(|(item, _)| *item == name)?;
