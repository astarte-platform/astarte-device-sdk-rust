...
impl TransportProvider {
    ...
    async fn store_credentials(
        ...
        let store_cert =
            atomic_write_private(certificate_file.as_ref(), certificate.as_bytes()).await;
        let store_key =
            atomic_write_private(private_key_file.as_ref(), private_key.secret_pkcs8_der()).await;
...
/// Atomically write to a file accessible only by the user
pub(crate) async fn atomic_write_private(dst: &Path, src: &[u8]) -> io::Result<()> {
    let mut tmp_file = dst.to_path_buf();

    if tmp_file.file_name().is_none() {
    ...
    match tokio::fs::remove_file(&tmp_file).await {
        ...
        Err(error) => {
            error!(%error, "couldn't remove temporary file");
    ...
    tokio::fs::rename(tmp_file, dst).await?;
...
mod tests {
    ...
    async fn should_write_safe_private() {
        ...
        atomic_write_private(&file, exp.as_bytes()).await.unwrap();
