...
impl WriteConnection {
    pub(crate) async fn connect(db_file: impl AsRef<Path>) -> Result<Self, SqliteError> {
        ...
        // check if the journal size hasn't been modified (default value -1) set it to 64 mebibytes
        // otherwise leave it as it is
        let current_journal_size: i32 = connection.get_pragma("journal_size_limit")?;
        if current_journal_size == -1 {
