...
impl WriteConnection {
    pub(crate) async fn connect(db_file: impl AsRef<Path>) -> Result<Self, SqliteError> {
        ...
        let mut connection = wrap_sync_call(|| Connection::open_with_flags(&db_file, flags))
        ...
        connection.pragma("busy_timeout", SQLITE_BUSY_TIMEOUT)?;
        connection.pragma("journal_mode", "WALL")?;

        Ok(connection)
    }

    fn pragma<V>(&self, pragma_name: &str, pragma_value: V) -> Result<(), SqliteError>
    where
        V: ToSql,
    {
    ...
    pub(super) fn delete_prop(&self, interface: &str, path: &str) -> Result<(), SqliteError> {
        wrap_sync_call(|| {
            ...
            let delted = statement
                .execute((interface, path))
                .map_err(SqliteError::Query)?;

            debug_assert!((0..=1).contains(&delted));
