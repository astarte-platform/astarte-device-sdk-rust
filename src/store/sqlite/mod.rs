...
fn from_stored_type(value: u8) -> Result<MappingType, Error<ValueError>> {
    let mapping_type = match value {
        ...
        0 | 15.. => {
            return Err(Error::new(ValueError::StoredType).set_message(format!("got {value}")));
        }
    };
        ...
        sqlite_store
            ...
            .acquire_writer(|writer| {
                writer
                    ...
                    .wrap_err_ctx(SqliteError::Option, "while running incremental VACUUM")
            })
            .await?;
...
fn deserialize_prop(stored_type: u8, buf: &[u8]) -> Result<AstarteData, Error<ValueError>> {
    ...
    AstarteData::try_from(value).wrap_err_ctx(ValueError::Decode, "invalid Astarte data")
}
