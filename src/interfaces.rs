...
impl Interfaces {
    ...
    pub(crate) fn validate(
        ...
        match self.get(interface.interface_name()) {
            Some(prev) => {
                ...
                interface
                    .validate_with(prev)
                    .wrap_err_msg(InterfaceError::Invalid, "with previous version")?;
    ...
    pub(crate) fn get_object<'a>(
        ...
        let interface = match interface.inner() {
            InterfaceTypeAggregation::DatastreamIndividual(_) => {
                return Err(Error::new(InterfaceError::Aggregation).set_ctx(format!(
            ...
            InterfaceTypeAggregation::Properties(_) => {
                return Err(Error::new(InterfaceError::InterfaceType).set_ctx(format!(
                    "for {interface_name}, expected {} bot got {}",
    ...
    pub(crate) fn get_individual<'a>(
        ...
        let mapping = interface.mapping(path).ok_or_else(|| {
            Error::new(InterfaceError::MappingNotFound).set_ctx(format!("for {interface}{path}"))
        })?;
