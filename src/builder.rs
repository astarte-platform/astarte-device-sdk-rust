...
use crate::state::{ConnStatus, ConnectionState, SharedState};
...
impl<S, C> DeviceBuilder<S, C> {
    ...
    /// Set a custom rustls [`ClientConfig`] for the connection.
    ///
    /// If this is not called, the default TLS configuration is used.
    pub fn client_tls(mut self, tls: ClientConfig) -> Self {
        self.tls = Some(tls);

        self
    }

    /// Disables server certificate and hostname verification.
    ///
    /// Connections can be intercepted. Use this option only for development and testing.
    pub fn insecure_tls(mut self) -> Result<Self, rustls::Error> {
        warn!("INSECURE: ignore TLS certificates");

        self.tls = Some(astarte_device_tls::insecure::insecure()?);
    ...
    C::Store<S>: StoreCapabilities,
{
    ...
    pub async fn build(mut self) -> Result<BuildRes<C, C::Store<S>>, AstarteError> {
        ...
        let (status_tx, status_rx) = tokio::sync::watch::channel(ConnStatus::default());

        let tls = match self.tls {
            Some(tls) => {
                trace!("using configured tls");

                tls
            }
            None => {
                trace!("using default tls configuration");

                astarte_device_tls::config()
                    .wrap_err_msg(ErrorKind::Tls, "while using default TLS configuration ")?
            }
        };
        ...
        let config = self.connection_config.configure(state).await?;

        let state = Arc::new(config.state);

        let registered = self.connection_config.is_registered(&state).await?;

        debug!(registered, "device registration status");

        state.set_device_status(registered);

        let (client_tx, client_rx) = tokio::sync::mpsc::channel(state.config.channel_size.get());
        let (events_tx, events_rx) = async_channel::bounded(state.config.channel_size.get());

        let (client_state, connection_state) = state.split();

        let client = DeviceClient::new(client_tx, events_rx, client_state, config.encoder);
...
type RegisterResult<Client, Connection> = Result<ControlFlow<(Client, Connection)>, AstarteError>;

/// Crate private connection implementation.
pub trait ConnectionConfig: Send + Sync + 'static {
    ...
    type Store<S>: Send + Sync + 'static
    where
        S: Send + Sync + 'static;

    /// Serializer for the client
    type Encoder: Clone + Send + Sync + 'static;

    /// Configures the connection.
    fn configure<S>(
        &mut self,
        config: SharedState<S>,
    ) -> impl Future<Output = Result<BuildConfig<Self::Store<S>, Self::Encoder>, AstarteError>> + Send
    where
        S: StoreCapabilities;
    ...
    ) -> impl Future<Output = RegisterResult<Self::Client, Self::Connection>> + Send
    where
        S: StoreCapabilities;
}
...
mod test {
    ...
    async fn should_build() {
        ...
        config
            ...
            .returning(|state| Ok(BuildConfig { state, encoder: MockEncoder::new() }));

        config
            .expect_is_registered::<MemoryStore>()
            .once()
            .in_sequence(&mut seq)
