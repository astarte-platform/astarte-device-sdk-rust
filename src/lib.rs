...
pub struct AstarteMqttTransport {
    ...
    eventloop: tokio::sync::Mutex<EventLoop>,
}
...
pub struct AstarteSdk {
    ...
    transport: Option<Arc<AstarteMqttTransport>>,
...
impl AstarteSdk {
    ...
    pub async fn connect(&mut self) -> Result<(), AstarteError> {
        ...
        let transport = AstarteMqttTransport {
            ...
            eventloop: tokio::sync::Mutex::new(eventloop),
        };
    ...
    pub async fn disconnect(&mut self) -> Result<(), AstarteError> {
        ...
        //self.transport.as_mut().ok_or(AstarteError::NoTransport)?.eventloop.lock().await.handle().close();
