...
async fn init() -> eyre::Result<(DeviceClient<SqliteStore>,  DeviceConnection<SqliteStore, Mqtt>)> {
    ...
    let store = SqliteStore::from_uri("sqlite://store.db").await?;
...
async fn main() -> eyre::Result<()> {
    ...
    let (mut client, mut connection) = init().await?;
...
#     DeviceClient, DeviceConnection, Error
# };
# use color_eyre::eyre;
# use serde::Deserialize;
# use tokio::task::JoinSet;
...
async fn receive_data(mut client: DeviceClient<SqliteStore>) -> eyre::Result<()> {
    loop {
        match client.recv().await {
            Ok(event) => {
                if let astarte_device_sdk::Value::Individual(data) = event.data {
...
#[tracing::instrument(skip_all)]
async fn send_individual(mut client: DeviceClient<SqliteStore>) -> eyre::Result<()> {
...
#[tracing::instrument(skip_all)]
async fn send_aggregate(mut client: DeviceClient<SqliteStore>) -> eyre::Result<()> {
    // send data every 1 sec
    let mut interval = tokio::time::interval(std::time::Duration::from_secs(1));
...
#[tracing::instrument(skip_all)]
async fn send_property(mut client: DeviceClient<SqliteStore>) -> eyre::Result<()> {
    let mut interval = tokio::time::interval(std::time::Duration::from_secs(1));
